...
--do node config
local autonode_config = function( files, config )
    --start output
    local out = '// oxyauto.js :: autogenerated by oxypanel\n'

    out = out .. [[module._autoconf = {
    database_host: ']] .. luawaconf.database.host .. [[',
    database_user: ']] .. luawaconf.database.user .. [[',
    database_password: ']] .. luawaconf.database.pass .. [[',
    database_name: ']] .. luawaconf.database.name .. [[',
    database_port: ]] .. luawaconf.database.port .. [[,
    ssh_key: ']] .. luawaconf.oxyngx.ssh_key .. [[',
    root: ']] .. config.root .. [['
...
luawa:processRequest()]]
    ...
    if not f then error( err ) end
    local status, err = f:write( autonode_config( autonode_files, _autoconf ) )
    if not status then error( err ) end
    print( '\toxyauto.js' )
