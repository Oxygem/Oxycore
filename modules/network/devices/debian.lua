...
local config = {
    ...
    commands = {
        ...
        update_packages = {
            permission = 'edit',
            actions = {
                { action = 'exec', command = 'apt-get update -y && apt-get upgrade -y', out = 'update' }
            }
        },
        --search packages
        search_packages = {
            ...
            actions = function( args )
                ...
                local out =  {
                    { action = 'exec', command = 'apt-cache search ' .. args.package, out = 'packages' }
                }
                return out
            end
        },
        ...
        install_package = {
            ...
            actions = function( args )
                ...
                local out =  {
                    { action = 'exec', command = 'apt-get install ' .. args.package .. ' -y', out = 'status' }
                }
                return out
            end
        }
